...
bool Npc::load()
{
    ...
    if (!scriptInterface) {
        ...
        /* TODO get any mandatory library files from configuration if necessary
...
bool Npc::loadFromXml()
{
    ...
    attackable = npcNode.attribute("can-be-attacked").as_bool();
    floorChange = npcNode.attribute("can-change-floor").as_bool();
    ...
    if ((attr = npcNode.attribute("can-be-pushed"))) {
        pushable = attr.as_bool();
    }

    if ((attr = npcNode.attribute("walk-interval"))) {
        walkTicks = pugi::cast<uint32_t>(attr.value());
    }

    if ((attr = npcNode.attribute("talk-delay"))) {
        talkDelay = pugi::cast<uint32_t>(attr.value());
    }

    if ((attr = npcNode.attribute("timeout"))) {
        timeout = pugi::cast<uint32_t>(attr.value());
    }

    if ((attr = npcNode.attribute("walk-radius"))) {
        masterRadius = pugi::cast<int32_t>(attr.value());
    }

    if ((attr = npcNode.attribute("listen-radius"))) {
        listenRadius = pugi::cast<int32_t>(attr.value());
    }

    if ((attr = npcNode.attribute("can-ignore-height"))) {
    ...
    setIdle(npcNode.attribute("idle").as_bool());
    ...
    /* TODO load NPC destination lists, expose via lua interface */

    for (auto itemNode : npcNode.child("shop").children()) {

        if ("shop-item" != std::string(itemNode.name())) {
            std::cout << "[Warning - Npc::loadFromXml] invalid child of trade element type(" << itemNode.name() << ") (" << filename << ")" << std::endl;
            continue;
        }
        ...
        if ((attr = itemNode.attribute("id"))) {
            item.itemId = pugi::cast<int32_t>(attr.value());

            if (item.itemId >= Item::items.size()) {
                std::cout << "[Warning - Npc::loadFromXml] unknown trade item ID(" << item.itemId << ") (" << filename << ")" << std::endl;
                continue;
            }

            item.realName = Item::items[item.itemId].name;
        } 

        if ((attr = itemNode.attribute("name"))) {
            item.realName = attr.as_string();

            if(!item.itemId) {
                auto ids = Item::items.nameToItems.equal_range(asLowerCaseString(item.realName));

                if (ids.first == Item::items.nameToItems.cend()) {
                    std::cout << "[Warning - Npc::loadFromXml] unknown trade item name(" << item.realName << ") (" << filename << ")" << std::endl;
                    continue;
                }

                item.itemId = ids.first->second;

                if (std::next(ids.first) != ids.second) {
                    std::cout << "[Warning - Npc::loadFromXml] Non-unique loot item name(" << item.realName <<") using ID(" << item.itemId << ") (" << filename << ")" << std::endl;
                }
            }
        }

        if (!(itemNode.attribute("id") || itemNode.attribute("name"))) {
            std::cout << "[Warning - Npc::loadFromXml] trade item requires either an id attribute or name attribute (" << filename << ")" << std::endl;
            continue;
        }

        if ((attr = itemNode.attribute("subtype"))) {
            item.subType = pugi::cast<int32_t>(attr.value());
        }

        if ((attr = itemNode.attribute("buy-price"))) {
            item.buyPrice = pugi::cast<int32_t>(attr.value());
        }

        if ((attr = itemNode.attribute("sell-price"))) {
            item.sellPrice = pugi::cast<int32_t>(attr.value());
        }

        if (!(itemNode.attribute("buy-price") || itemNode.attribute("sell-price"))) {
            std::cout << "[Warning - Npc::loadFromXml] trade item requires either a buy attribute or sell attribute (" << filename << ")" << std::endl;
            continue;
        }
...
void NpcScriptInterface::registerFunctions()
{
    ...
    registerMethod("Npc", "getListenRadius", NpcScriptInterface::luaNpcGetListenRadius);
    registerMethod("Npc", "getTalkDelay", NpcScriptInterface::luaNpcGetTalkDelay);
    registerMethod("Npc", "getTimeout", NpcScriptInterface::luaNpcGetTimeout);

    registerMethod("Npc", "getShopItems", NpcScriptInterface::luaNpcGetShopItems);
...
int NpcScriptInterface::luaOpenShopWindow(lua_State* L)
{
    ...
    while (lua_next(L, -2) != 0) {
        ...
        item.buyPrice = getField<uint32_t>(L, tableIndex, "sellPrice");
        item.sellPrice = getField<uint32_t>(L, tableIndex, "buyPrice");
    ...
    player->setShopOwner(npc, sellCallback, buyCallback);
...
int NpcScriptInterface::luaNpcGetListenRadius(lua_State* L) {

    Npc* npc = getUserdata<Npc>(L, 1);
    if (npc) {
        lua_pushnumber(L, npc->listenRadius);
    } else {
        lua_pushnil(L);
    }
...
int NpcScriptInterface::luaNpcGetTimeout(lua_State* L) {
    Npc* npc = getUserdata<Npc>(L, 1);
    if (npc) {
        lua_pushnumber(L, npc->timeout);
    } else {
        lua_pushnil(L);
    }
...
int NpcScriptInterface::luaNpcGetTalkDelay(lua_State* L) {
    Npc* npc = getUserdata<Npc>(L, 1);
    if (npc) {
        lua_pushnumber(L, npc->talkDelay);
    } else {
        lua_pushnil(L);
    }
...
void pushShopItems(lua_State* L, const std::list<ShopInfo>& items) {
    ...
    for (const auto& item : items) {
        ...
        LuaScriptInterface::setField(L, "id", item.itemId);
        LuaScriptInterface::setField(L, "subtype", item.subType);
        LuaScriptInterface::setField(L, "buyPrice", item.buyPrice);
        LuaScriptInterface::setField(L, "sellPrice", item.sellPrice);
        LuaScriptInterface::setField(L, "name", item.realName);

        LuaScriptInterface::setMetatable(L, -1, "ShopInfo");

        lua_rawseti(L, -2, ++index);
    }
}

int NpcScriptInterface::luaNpcGetShopItems(lua_State* L) {
    Npc* npc = getUserdata<Npc>(L, 1);
    if (npc) {
        pushShopItems(L, npc->shop);
    } else {
        lua_pushnil(L);
    }
...
{
    /* TODO get data directory from config; use an actual path merge here */
    loaded = scriptInterface->loadFile("data/" + file, npc) == 0;
    if (!loaded) {
